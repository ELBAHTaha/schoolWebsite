...
<div style="min-height:100vh;display:grid;place-items:center;padding:1rem;">
    <form method="POST" action="{{ route('login.attempt') }}" class="card" style="width:min(420px,100%);">
        @csrf
        <h1 style="margin-top:0;">Connexion</h1>

        <label for="email">Email</label>
        <input id="email" name="email" type="email" value="{{ old('email') }}" required style="width:100%;padding:.6rem;margin:.4rem 0 1rem;border:1px solid #d1d5db;border-radius:.4rem;">
        ...
        @if($errors->any())
            <div style="color:#b91c1c;margin-bottom:1rem;">{{ $errors->first() }}</div>
        @endif
