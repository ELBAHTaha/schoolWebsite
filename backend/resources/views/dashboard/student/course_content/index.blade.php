...
@section('title', 'Student - Course Content')
            ...
            <table style="width:100%;border-collapse:separate;border-spacing:0;min-width:600px;">
                ...
                @foreach($materials as $material)
                    <tr style="border-bottom:1px solid #e2e8f0;">
                        <td style="padding:.8rem .7rem;">{{ $material->title }}</td>
                        <td style="padding:.8rem .7rem;">{{ $material->schoolClass?->name ?? '-' }}</td>
                        <td style="padding:.8rem .7rem;">
                            <a href="{{ asset('storage/'.$material->file_path) }}" target="_blank" rel="noopener">Télécharger</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    <div style="padding:2rem 1.2rem">
        ...
        @else
            <table style="width:100%;border-collapse:separate;border-spacing:0;min-width:600px;">
                ...
                @foreach($pdfs as $pdf)
                    <tr style="border-bottom:1px solid #e2e8f0;">
                        <td style="padding:.8rem .7rem;">{{ $pdf->title }}</td>
                        <td style="padding:.8rem .7rem;">{{ $pdf->schoolClass?->name ?? '-' }}</td>
                        <td style="padding:.8rem .7rem;">
                            <a href="{{ route('pdfs.download', $pdf) }}">Télécharger</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
