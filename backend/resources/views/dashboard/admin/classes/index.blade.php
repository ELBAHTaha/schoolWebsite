...
<div style="max-width:1400px;margin:0 auto;padding:2.5rem 2rem 2rem 2rem;background:#f8fafc;min-height:100vh;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1.2rem;margin-bottom:2.2rem;">
        <div style="padding:1.3rem 1.2rem;display:flex;align-items:center;gap:.8rem">
            ...
            <div>
                <div style="font-size:1.5rem;font-weight:700;">{{ $stats['total'] ?? $classes->count() }}</div>
        ...
        <div style="padding:1.3rem 1.2rem;display:flex;align-items:center;gap:.8rem">
            ...
            <div>
                <div style="font-size:1.5rem;font-weight:700;">{{ $stats['inactive'] ?? '-' }}</div>
        ...
        <div style="padding:1.3rem 1.2rem;display:flex;align-items:center;gap:.8rem">
            ...
            <div>
                <div style="font-size:1.5rem;font-weight:700;">{{ $stats['special'] ?? '-' }}</div>
    ...
    <div class="class-card"
        ...
        data-name-display="{{ $class->name }}"
        data-description="{{ $class->description }}"
        data-professor="{{ strtolower($class->professor?->name ?? '') }}"
        data-professor-id="{{ $class->professor_id ?? '' }}"
        data-room="{{ strtolower($class->room?->name ?? '') }}"
        data-room-id="{{ $class->room_id ?? '' }}"
        data-price-1="{{ $class->price_1_month ?? '' }}"
        data-price-3="{{ $class->price_3_month ?? '' }}"
        data-price-6="{{ $class->price_6_month ?? '' }}"
        data-update-url="{{ route('admin.classes.update', $class) }}"
        style="padding:1.5rem 1.3rem;display:flex;flex-direction:column;gap:.7rem;position:relative;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
                <div style="font-size:1.18rem;font-weight:700;color:#1e293b;">{{ $class->name }}</div>
            </div>
        </div>
        <div style="margin-top:.3rem;display:flex;gap:.7rem;flex-wrap:wrap;align-items:center;">
            @if($class->professor)
                <span style="background:#2563eb;color:#fff;padding:.2rem .7rem;border-radius:.5rem;font-size:.93rem;font-weight:500;">{{ $class->professor->name }}</span>
            @endif
            @if($class->room)
                <span style="background:#34d399;color:#fff;padding:.2rem .7rem;border-radius:.5rem;font-size:.93rem;font-weight:500;">{{ $class->room->name }}</span>
            @endif
        </div>
        <div style="margin-top:.4rem;display:flex;flex-direction:column;gap:.25rem;color:#475569;font-size:.92rem;">
            <div>1 month: {{ $class->price_1_month !== null ? number_format((float) $class->price_1_month, 2) . ' DH' : '-' }}</div>
            <div>3 months: {{ $class->price_3_month !== null ? number_format((float) $class->price_3_month, 2) . ' DH' : '-' }}</div>
            <div>6 months: {{ $class->price_6_month !== null ? number_format((float) $class->price_6_month, 2) . ' DH' : '-' }}</div>
        </div>
...
<div id="modalBackdrop" class="modal-bg">
    <form method="POST" action="{{ route('admin.classes.store') }}" class="modal-card">
        @csrf
        ...
        <input name="price_1_month" type="number" step="0.01" min="0" placeholder="Price 1 month (DH)" class="modal-input">
        <input name="price_3_month" type="number" step="0.01" min="0" placeholder="Price 3 months (DH)" class="modal-input">
        <input name="price_6_month" type="number" step="0.01" min="0" placeholder="Price 6 months (DH)" class="modal-input">
        <button class="create-btn" style="width:100%">Créer</button>
    </form>
</div>

<!-- --- Modal Edit Classe --- -->
<div id="editModalBackdrop" class="modal-bg">
    <form method="POST" id="editForm" class="modal-card">
        @csrf
        @method('PUT')
        <button type="button" id="closeEditFormBtn" class="modal-close">×</button>
        <div class="modal-title">Modifier la classe</div>
        <input id="editName" name="name" placeholder="Nom de la classe" required class="modal-input">
        <input id="editDescription" name="description" placeholder="Description" class="modal-input">
        <select id="editProfessor" name="professor_id" class="modal-input">
            <option value="">Professeur</option>
            @foreach($professors as $professor)
                <option value="{{ $professor->id }}">{{ $professor->name }}</option>
            @endforeach
        </select>
        <select id="editRoom" name="room_id" class="modal-input">
            <option value="">Salle</option>
            @foreach($rooms as $room)
                <option value="{{ $room->id }}">{{ $room->name }}</option>
            @endforeach
        </select>
        <input id="editPrice1" name="price_1_month" type="number" step="0.01" min="0" placeholder="Price 1 month (DH)" class="modal-input">
        <input id="editPrice3" name="price_3_month" type="number" step="0.01" min="0" placeholder="Price 3 months (DH)" class="modal-input">
        <input id="editPrice6" name="price_6_month" type="number" step="0.01" min="0" placeholder="Price 6 months (DH)" class="modal-input">
        <button class="create-btn" style="width:100%">Enregistrer</button>
    </form>
</div>
...
const editPrice1 = document.getElementById("editPrice1")
const editPrice3 = document.getElementById("editPrice3")
const editPrice6 = document.getElementById("editPrice6")

document.getElementById("closeEditFormBtn").onclick=()=>{editModal.style.display="none"}
...
document.querySelectorAll('.edit-btn').forEach((btn)=>{
    btn.addEventListener('click', (e)=>{
        ...
        if(!card){return}
        editForm.action = card.dataset.updateUrl
        editName.value = card.dataset.nameDisplay || ''
        editDescription.value = card.dataset.description || ''
        editProfessor.value = card.dataset.professorId || ''
        editRoom.value = card.dataset.roomId || ''
        editPrice1.value = card.dataset.price1 || ''
        editPrice3.value = card.dataset.price3 || ''
        editPrice6.value = card.dataset.price6 || ''
