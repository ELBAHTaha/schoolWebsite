...
<div style="max-width:1400px;margin:0 auto;padding:2.5rem 2rem 2rem 2rem;background:#f8fafc;min-height:100vh;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.5rem;margin-bottom:2.2rem;">
        <div style="padding:2rem 1.2rem;display:flex;flex-direction:column;align-items:center;transition:box-shadow .2s">
            <strong style="font-size:2.2rem;color:#2563eb;">{{ $stats['total_payments'] }}</strong>
            <small style="margin-top:.5rem;color:#64748b;font-size:1.1rem;">Paiements enregistrés</small>
        </div>
        <div style="padding:2rem 1.2rem;display:flex;flex-direction:column;align-items:center;transition:box-shadow .2s">
            <strong style="font-size:2.2rem;color:#2563eb;">{{ $stats['collected_this_month'] }} MAD</strong>
            <small style="margin-top:.5rem;color:#64748b;font-size:1.1rem;">Montant collecté ce mois</small>
        </div>
        <div style="padding:2rem 1.2rem;display:flex;flex-direction:column;align-items:center;transition:box-shadow .2s">
            <strong style="font-size:2.2rem;color:#2563eb;">{{ $stats['pending_payments'] }}</strong>
            <small style="margin-top:.5rem;color:#64748b;font-size:1.1rem;">Paiements à suivre</small>
        </div>
        <div style="padding:2rem 1.2rem;display:flex;flex-direction:column;align-items:center;transition:box-shadow .2s">
            <strong style="font-size:2.2rem;color:#2563eb;">{{ $stats['paid_this_month'] }}</strong>
    ...
    <div style="padding:2rem 1.2rem">
        ...
        <div style="display:flex;gap:1rem;flex-wrap:wrap;">
            <a class="btn" style="background:linear-gradient(135deg,#2563eb,#0ea5e9);color:white;font-weight:600;padding:.7rem 1.5rem;border-radius:.7rem;font-size:1.08rem;box-shadow:0 2px 8px 0 rgba(37,99,235,0.10);border:none;" href="{{ route('secretary.students.create') }}">Nouvel étudiant</a>
            <a class="btn" style="background:linear-gradient(135deg,#2563eb,#0ea5e9);color:white;font-weight:600;padding:.7rem 1.5rem;border-radius:.7rem;font-size:1.08rem;box-shadow:0 2px 8px 0 rgba(37,99,235,0.10);border:none;" href="{{ route('secretary.payments.create') }}">Nouveau paiement</a>
            <a class="btn" style="background:linear-gradient(135deg,#2563eb,#0ea5e9);color:white;font-weight:600;padding:.7rem 1.5rem;border-radius:.7rem;font-size:1.08rem;box-shadow:0 2px 8px 0 rgba(37,99,235,0.10);border:none;" href="{{ route('secretary.payments.index') }}">Voir tous les paiements</a>
        </div>
    </div>
</div>
@endsection
